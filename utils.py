...
def collect_user_details(request_header):
    ...
    if search_option == 2:
        ...
        # exclude pincodes from district search to API call optimisation while searching multiple pincodes
    ...
    # commenting out this section as it's not required
    #    minimum_slots = input(
    ...
    # CAREFUL WITH THIS OPTION! AUTO-BOOKING WILL BOOK THE FIRST AVAILABLE CENTRE, DATE, AND A RANDOM SLOT! =====" )
    auto_book = "yes-please"

    print("\n================================= Captcha Automation =================================\n")
    #    print("======== Caution: This will require a paid API key from anti-captcha.com =============")

    captcha_automation = input("Do you want to automate captcha autofill? (api/ai/n) Default n: ")
        ...
        while valid_captcha:
            ...
            if resp.status_code == 401:
                ...
                print(
                    ...
                    "           Go to https://cowin.gov.in and download the appointment slip from there"
                )
        ...
        if not slots_available:
            ...
            for i in range(0, len(options)):
                ...
                if not all_slots_of_a_center:
                    continue
                # For better chances of booking, use random slots of a particular center This will help if too many
                # folks are trying for same region at the same time. Everyone will have better chances of booking
                # otherwise everyone will look for same slot of same center at a time. Randomized slots selection is
                # maximizing chances of booking
                #                random.shuffle(all_slots_of_a_center)  # in-place modification
                loop_count = 0
                for selected_slot in all_slots_of_a_center:
                    # if have spent too much time in loop iteration then means we are looking at stale information
                    # about centers & slots. so we should re-calculate this information while ending this loop more
...
def vaccine_dose2_duedate(vaccine_type, dose1_date):
    ...
    """
    #    covishield due days = 84
    #    covaxin due days = 28
    #    sputnikV due days= 21
